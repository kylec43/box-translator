use tauri::{Window, Size, PhysicalSize, PhysicalPosition, Position};
...
impl WindowWrapper {
    ...
    pub fn set_physical_size(&mut self, width: u32, height: u32) -> &mut WindowWrapper {
        let _ = self.window.set_size(Size::Physical(PhysicalSize { width, height }));
        self
    }

    pub fn set_physical_position(&mut self, x: i32, y: i32) -> &mut WindowWrapper {
        let _ = self.window.set_position(Position::Physical(PhysicalPosition { x, y }));
        self
    }

    pub fn set_physical_min_size(&mut self, width: u32, height: u32) -> &mut WindowWrapper {
        let _ = self.window.set_min_size(Some(Size::Physical(PhysicalSize { width, height })));
        self
    }
